...
class diff_match_patch:
  ...
  def diff_compute(self, text1, text2, checklines):
    ...
    # Perform a real diff.
    if checklines and len(text1) + len(text2) < 250:
  ...
  def diff_linesToChars(self, text1, text2):
    ...
    def diff_linesToCharsMunge(text):
      ...
      while lineEnd < len(text) - 1:
        ...
        if line in lineHash:
          chars += unichr(lineHash[line])
        else:
          lineArray.append(line)
          lineHash[line] = len(lineArray) - 1
          chars += unichr(len(lineArray) - 1)
      return chars
  ...
  def diff_map(self, text1, text2):
    ...
    ms_end = time.time() + self.Diff_Timeout  # Don't run for too long.
      ...
      if doubleEnd:
        ...
        for k in xrange(-d, d + 1, 2):
          if k == -d or k != d and v2[k - 1] < v2[k + 1]:
            ...
            x = v2[k - 1] + 1
          y = x - k
          footstep = str(len(text1) - x) + "," + str(len(text2) - y)
          if not front and footstep in footsteps:
            done = True
          if front:
            footsteps[footstep] = d
          while (not done and x < len(text1) and y < len(text2) and
                 text1[len(text1) - x - 1] == text2[len(text2) - y - 1]):
            x += 1
            y += 1
            footstep = str(len(text1) - x) + "," + str(len(text2) - y)
            if not front and footstep in footsteps:
  ...
  def diff_halfMatch(self, text1, text2):
    ...
    hm1 = diff_halfMatchI(longtext, shorttext,
                          int(math.ceil(len(longtext) / 4)))
    # Check again based on the third quarter.
    hm2 = diff_halfMatchI(longtext, shorttext,
                          int(math.ceil(len(longtext) / 2)))
  ...
  def diff_cleanupSemantic(self, diffs):
    ...
    lastequality = None  # Always equal to equalities[len(equalities)-1][1]
    ...
    while pointer < len(diffs):
      if diffs[pointer][0] == self.DIFF_EQUAL:  # equality found
        ...
        if (lastequality != None and (len(lastequality) <= length_changes1) and
          ...
          diffs[equalities[len(equalities) - 1] + 1] = (self.DIFF_INSERT,
  ...
  def diff_cleanupSemanticLossless(self, diffs):
    ...
    def diff_cleanupSemanticScore(one, two, three):
      """Given three strings, compute a score representing whether the two
      internal boundaries fall on word boundaries.
      Closure, but does not reference any external variables.
      ...
      if one[-1].isspace() or two[0].isspace():
        score += 1
      if two[-1].isspace() or three[0].isspace():
        score += 1
      return score
  ...
  def diff_addIndex(self, diffs):
    """Add an index to each tuple, represents where the tuple is located in
    text2.
  ...
  def diff_prettyHtml(self, diffs):
    ...
    Returns:
      HTML representation.
    """
    self.diff_addIndex(diffs)
    html = []
    for (m, t, i) in diffs:
      t = t.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
      t = t.replace("\n", "&para;<BR>")
      if m == self.DIFF_INSERT:
        html.append("<INS STYLE=\"background:#E6FFE6;\" TITLE=\"i=%i\">%s</INS>" %
            (i, t))
      elif m == self.DIFF_DELETE:
        html.append("<DEL STYLE=\"background:#FFE6E6;\" TITLE=\"i=%i\">%s</DEL>" %
            (i, t))
      elif m == self.DIFF_EQUAL:
        html.append("<SPAN TITLE=\"i=%i\">%s</SPAN>" % (i, t))
    return "".join(html)
  ...
  def diff_toDelta(self, diffs):
    ...
    for (op, data) in diffs:
      if op == self.DIFF_INSERT:
        txt.append("+" + urllib.quote(data.encode("utf-8"), " =;',/~!@#$&*():?") + "\t")
      elif op == self.DIFF_DELETE:
        txt.append("-%d\t" % len(data))
  ...
  def match_bitap(self, text, pattern, loc):
    ...
    for d in xrange(0, len(pattern)):
      ...
      if text[finish] == pattern[len(pattern) - 1]:
        rd[finish] = (1 << (d + 1)) - 1
      else:
        rd[finish] = (1 << d) - 1
  ...
  def patch_make(self, a, b=None, c=None):
    ...
    if b is None:
      ...
      text2 = b
      if c is None:
        diffs = self.diff_main(text1, text2, True)
        if len(diffs) > 2:
          self.diff_cleanupSemantic(diffs)
        ...
        while len(bigpatch.diffs) != 0:
          ...
          while (len(bigpatch.diffs) != 0 and
            ...
            if diff_type == self.DIFF_INSERT:
              ...
              patch.diffs.append(bigpatch.diffs.shift())
              empty = False
            else:
  ...
  def patch_fromText(self, textline):
    ...
    Returns:
      Array of patch objects or None if invalid.
    ...
    while len(text) != 0:
      ...
      while len(text) != 0:
        ...
        if sign == '+':
          ...
          # WTF?
          raise ValueError, "Invalid patch mode: '" + sign + "'\n" + line
        del text[0]
    return patches
